...
distributed. But, \manuelComment{as done by L\"{a}ssig in a very confusing way}, we
...
a) In a genome of length $\Nns \sim 5\times 10^6$ there must be at most one
...
\manuelComment{In a single statement with no derivation L\"{a}ssig uses the
...
Since we have $\moment{\xi_x(t) \xi_x(t')} = \delta(t - t')$, it must be the
...
sense of direction.'' The magnitude of the jump depends on where are you
standing on the real line, but it is equally likely to jump left or right. This
can be clearly seen if one takes the ``spread the butter'' approach. There is
easy to note that indeed the magnitude of the jump depends on where are you
standing in frequency space, but the magnitude applies to both directions left
and right. In the case of this directional term $- {1 \over 2} \partial_x (1 -
2x) P$ breaks the symmetry of these jumps. The closer you get to 1 or 0, the
more you jump towards that direction. That is why at $x = {1 \over 2}$ this term
vanishes.}
...
function $\DF$ equation is fo the form \manuelComment{The set of notes deriving
this result will be also written later on.}
\begin{equation}
...
\manuelComment{Following L\"{a}ssig, Kimura \& Rice I will ignore for now the
...
\end{equation}
\manuelComment{L\"{a}ssig has a typo on Eq. 45 where he writes ${1 \over N}$ rather
than ${1 \over 2N}$.}
...
L\"{a}ssig then goes to discuss how to apply this relationship to regulatory
...
\includepdf[pages=-]{lassig_2007.pdf}
