...
in \secref{sec_master_eq} the master equation is a statement about the time
...
did for \eref{eq_transition_short_time}, obtaining
\begin{equation}
	P(f_o + r, t_o + \Dt \mid f_o, t_o) =
	\delta(r) \left[
	1 - a^{(0)}(f_o, t_o) \Dt
...
Rather than trying to solve directly this equation we can take the same approach
...
distribution first we set the time derivative of \eref{eq_kolmogorov_backward}
to zero
\begin{equation}
