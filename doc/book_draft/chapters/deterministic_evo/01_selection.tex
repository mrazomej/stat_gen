...
\end{equation}
This is the solution of the logistic equation. \fref{fig_deterministic_select}
shows this solution for different selection coefficients. We can se that in
this deterministic regime as long as there is a fitness difference, i.e. as
long as $s \neq 0$, the fate of the population is set. If $s > 0$ allele $A$
will go into fixation. Otherwise if $s < 0$ the allele will go extinct.
...
\begin{figure}[h!]
  ...
  \label{fig_deterministic_select}
\end{figure}