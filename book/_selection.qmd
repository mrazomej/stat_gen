...
Furthermore, thinking about microbes whose populations can be on the order of
$10^6-10^9$ cells allow us to simplify our calculations. Rather than keeping
track of each individual life's history, we can approximate the population
number as a **continuous variable**. This means that when discussing the number
of organisms in the population, we won't worry about what it means to have
"fractional organisms."

![One-locus two-allele selection. Schematic of the toy model for selection only
case.](./figures/placeholder.jpg){#fig-selection_schematic}

For the simplest case of one-locus two-alleles, let us label the two alleles as
$A$ and $a$ as depicted in @fig-selection_schematic. Let $N_A(t)$ and $N_a(t)$
be the corresponding number of organisms carrying each allele at time $t$. We
want to compute the number of organisms with, let's say, allele $A$ after a time
$\Delta t$. For the frequency-independent selection, the only factor that
matters is the current number of organisms with allele $N_A(t)$ and their
corresponding growth rate $\lambda_A$. As discussed before, we will use this
growth rate as a proxy for fitness; this is sometimes called Malthusian fitness
after Thomas Robert Malthus---famous scholar whose essay on population growth
inspired both Darwin and Wallace. This growth rate has units of $\lambda_A [=]
\frac{\#\text{organisms}}{\text{time}}$. Therefore, the number of organisms at
time $t + \Delta t$ can then be computed as
$$
N_A(t + \Delta t) = \overbrace{N_A(t)}^\text{current number of organisms}
+ \overbrace{\lambda_A N_A(t) \Delta t}^{\text{change in number in }\Delta t}.
$${#eq-NA_t_delta_t}
...
$$
\frac{d N_A}{dt} = \lambda_A N_A(t).
$${#eq-expo_growth}
We can define an equivalent equation for allele $a$ with fitness $\lambda_a$.
...
$$
\frac{dN_\text{tot}}{dt} = 
N_\text{tot}(t) \left[ \lambda_A x(t) + \lambda_a (1 - x(t)) \right].
$${#eq-dNot_factorized}
From this, we can see the average fitness $\bar{f}(t)$ is of the form
$$
\bar{\lambda}(t) = \lambda_A x(t) + \lambda_a (1 - x(t)).
$${#eq-pop_fitness_def}
...
gives
$$
\frac{dx}{dt} = \frac{\left[\lambda_A N_A(t)\right] 
...
to obtain
$$
\frac{dx}{dt} = \frac{\lambda_A N_A(t) - 
N_A(t) [\lambda_a x(t) + \lambda_a (1 - x(t))]}{N_\text{tot}(t)}.
...
$$
\frac{dx}{dt} = \lambda_A x(t) - x(t) \left[\lambda_A x(t) + 
\lambda_a (1 - x(t))\right].
$${#eq-dx_Dt_chainrule_4}
Grouping terms by the fitness values results in
$$
\frac{dx}{dt} = \lambda_A x(t)(1 - x(t)) - \lambda_a x(t)(1 - x(t)) 
= s x(t)(1 - x(t)),
$${#eq-freq_dynamics_selection}
where we defined $s = \lambda_A - \lambda_a$, the fitness difference between
alleles, also known as the *selection coefficient*. @eq-freq_dynamics_selection
is telling us that for the population to change its relative composition---what
we define as the evolution of the population---there must be a difference in
fitness (growth rates) between the competing organisms. 
